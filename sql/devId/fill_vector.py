...
def upload_vectors(vectors, success, total_uploading):
	for idx, (dayst, devMAC, avgPwr, varPwr, maxPwr, minPwr, count, duty, crossCorr, pOcc, ct5, \
		...
		aws_c.execute('insert into temp_dat_occ_vector (dayst, deviceMAC, ' \
...
aws_c.execute('select deviceType, count(*) as count from mr_dat_occ_vector group by deviceType order by count desc;')
typeList = aws_c.fetchall()
most_vectors = typeList[0][1]

total_needed = sum([most_vectors-x[1] for x in typeList])
print('Total needed: ' + str(total_needed))

success = 0
total_uploading = len(typeList) * most_vectors
print('Total uploading: ' + str(total_uploading) + ' (' + str(len(typeList)) + ' categories, ' + str(most_vectors) + ' maxVal)')
...
for devType, count in typeList:
	...
	aws_c.execute('select * from mr_dat_occ_vector where deviceType=\'' + devType + '\';')
	vectors = aws_c.fetchall()

	while((new_total + count) <= most_vectors):
		success = upload_vectors(vectors, success, total_uploading)
		new_total += count

	still_needed = most_vectors - new_total

	success = upload_vectors(sample(vectors, still_needed), success, total_uploading)
