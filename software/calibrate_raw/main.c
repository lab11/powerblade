 ...
 * calibrate_raw
 *
 * The purpose of this app is to determine the DC offset of the current and voltage amplifiers.
 * Run this app while powered from the programmer, either insert a breakpoint before the uart_send
 * or view the reported values in the BLE app
 *
 * Reported values at 1Hz:
 * 	   PowerBlade ID:	1 byte
 * 	         Ioffset:	4 bytes		(average since power on)
 * 	         Voffset:	4 bytes		(average since power on)
 * 	     Ioffset_min:	1 byte		(over last 1s)
 * 	     Ioffset_max: 	2 bytes		(over last 1s)
 * 	     Voffset_min:	2 bytes		(over last 1s)
 * 	     Voffset_max:	4 bytes		(over last 1s)
 */
